...
class ItemAnalysisController extends Controller
{
    private function connection(Request $request): Connection
    {
        ...
        if (Gate::has('item-analysis.view')) {
            Gate::forUser($request->user())->authorize('item-analysis.view', [$code]);
        }

        return $db;
    }

    private function canGenerate(Connection $db, Request $request): bool
    {
        $code = strtoupper(trim((string) $request->session()->get('school_code', '')));
        if (Gate::has('item-analysis.generate')) {
            return Gate::forUser($request->user())->allows('item-analysis.generate', [$code]);
        }
        // Conservative write access; never trust a role posted by the browser.
        return $db->table('login as l')->join('login_type as t', 't.id', '=', 'l.login_type_id')
            ->where('l.id', $request->user()->getAuthIdentifier())->where('t.code_type', 'Administrator')->exists();
    }
    ...
    public function options(Request $request): JsonResponse
    {
        ...
        return response()->json([
            ...
            'can_generate' => $this->canGenerate($db, $request),
        ]);
    }
    ...
    public function preview(Request $request): JsonResponse
    {
        ...
        $report['saved'] = false;
        $report['updated_questions'] = 0;
        $report['message'] = 'Preview only. No history or questions were changed.';
    ...
    {
        $db = $this->connection($request);
        abort_unless($this->canGenerate($db, $request), 403, 'You do not have permission to save item analysis.');
        $input = $this->input($request);
        // Subject-row serialization avoids duplicate history inserts by simultaneous generations.
        // Locking/rollback require transactional engines (e.g. InnoDB) on all involved tables.
        $report = $db->transaction(function () use ($db, $request, $input): array {
            $subject = $db->table('bs_topic')->where('id', $input['bs_topic_id'])
                ->where('bs_course_id', $input['bs_course_id'])->lockForUpdate()->first(['id']);
            abort_unless($subject, 422, 'The selected subject does not belong to the exam package.');
            $context = $this->context($db, $request, $input);
            $report = $this->calculate($db, $context, $input['metric_mode'], true);
            $eligible = $report['items']->filter(fn ($item) => $item['appearances'] > 0 && $item['decision'] !== null);
            if ($eligible->isEmpty()) {
                throw ValidationException::withMessages(['results' => 'No analyzable questions exist in the selected groups. Nothing was saved.']);
            }
            $existing = $db->table('bs_iar_history')->where('analyzed_by_id', $context->analyzed_by_id)
                ->where('bs_exam_session_id', $context->bs_exam_session_id)->where('bs_course_id', $context->bs_course_id)
                ->where('bs_topic_id', $context->bs_topic_id)->where('pct', $context->pct)->orderBy('id')->lockForUpdate()->first(['id', 'date_analyzed']);
            $created = ! $existing;
            $context->id = $existing->id ?? (string) Str::uuid();
            if ($created) {
                $db->table('bs_iar_history')->insert([
                    'id' => $context->id, 'date_analyzed' => $context->date_analyzed, 'analyzed_by_id' => $context->analyzed_by_id,
                    'company_id' => '', 'bs_exam_session_id' => $context->bs_exam_session_id,
                    'bs_course_id' => $context->bs_course_id, 'bs_topic_id' => $context->bs_topic_id, 'pct' => $context->pct,
                ]);
            } else {
                // Preserve original history date, matching legacy deduplication behavior.
                $context->date_analyzed = $existing->date_analyzed;
            }
            foreach ($eligible->groupBy('decision') as $decision => $items) {
                foreach ($items->pluck('id')->chunk(500) as $ids) {
                    $db->table('bs_quest')->whereIn('id', $ids->all())->where('bs_topic_id', $context->bs_topic_id)
                        ->where('active', 'Y')->update(['validated' => 'Y', 'decision' => $decision]);
                }
            }
            $report['history'] = $context;
            $report['saved'] = true;
            $report['history_created'] = $created;
            $report['updated_questions'] = $eligible->count();
            $report['message'] = $created ? 'Analysis saved, history created, and eligible questions marked validated.'
                : 'Analysis saved and eligible questions marked validated. The existing history reference was reused.';
    ...
    private function calculate(Connection $db, object $history, string $mode, bool $lockQuestions = false): array
    {
        $pct = $history->pct;
        ...
        if ($lockQuestions) { $questionQuery->lockForUpdate(); }
        ...
        $warnings = ['Calculated from current exam data and active questions. Saved history references do not store an immutable per-item snapshot or calculation mode.',
