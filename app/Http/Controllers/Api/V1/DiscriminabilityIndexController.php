...
class DiscriminabilityIndexController extends Controller
{
    private function connection(Request $request): Connection
    {
        ...
        if (Gate::has('discriminability-index.view')) {
            Gate::forUser($request->user())->authorize('discriminability-index.view', [$code]);
        }
    ...
    private function calculate(Connection $db, object $history, string $mode): array
    {
        ...
        if ($groupSize === 0) {
            $warnings[] = 'Too few scored students for the selected group percentage. DF and DS are unavailable.';
        }
        ...
        $items = $questions->values()->map(function ($question, $i) use ($upper, $lower, $groupSize, $mode): array {
            ...
            return ['id' => $question->id, 'index' => $i + 1, 'question' => $this->decode($question->quest_text), 'appearances' => $appears,
                ...
                'df_remarks' => $difficulty, 'ds' => $ds === null ? null : round($ds, 2), 'ds_remarks' => $discrimination, 'options' => $options];
        });
